...
const EditProductModal = ({ product, isOpen, onClose, onSave }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    // Use mock API to update product
    try {
      const response = await mockApi.updateProduct(product._id, {
      ...
      if (response.product) {
        console.log('Product updated via mock API:', response.product);
        // Transform response to match frontend expectations
        const savedProduct = {
          ...response.product,
          name: response.product.title,
          quantity: response.product.stock
        };
        onSave(savedProduct);
        return;
      } else {
